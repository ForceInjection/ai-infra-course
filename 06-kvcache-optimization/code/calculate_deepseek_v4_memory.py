...
def calculate_deepseek_v4_memory(args):
    ...
    if swa_only:
        print(f"  [WARN] {len(swa_only)} SWA-only layers (compress_ratio=0) — using sliding-window-only KV (W={W}).")
    L_swa = len(swa_only)
    if other:
    ...
    def kv_per_layer_bf16(S, C, has_indexer):
        window = min(S, W)
        compressed = max(0, S - window) / C  # only tokens outside sliding window are compressed
        base = b_kv * (window + compressed) * d_kv
        if has_indexer:
            base += b_kv * compressed * d_idx
        return base

    def swa_per_layer(S, b):
        """Sliding-window-only layer KV (no compression)."""
        return b * min(S, W) * d_kv

    for label, S_val in sample_lengths:
        m_c4a = kv_per_layer_bf16(S_val, C_c4a, True)
        m_c128a = kv_per_layer_bf16(S_val, C_c128a, False)
        m_swa = swa_per_layer(S_val, b_kv)
        total_kv = L_c4a * m_c4a + L_c128a * m_c128a + L_swa * m_swa
    ...
    mem_swa_1M = swa_per_layer(max_S, b_kv)
    total_kv_1M_bf16 = L_c4a * mem_c4a_1M + L_c128a * mem_c128a_1M + L_swa * mem_swa_1M
    kv_per_token_avg = total_kv_1M_bf16 / max_S  # asymptotic per-token cost

    mla_per_layer = (d_kv + cfg.get("qk_rope_head_dim", 64)) * 2  # classic MLA: K+V, no indexer
    ...
    def kv_per_layer_mixed(S, C, has_indexer):
        window = min(S, W)
        compressed = max(0, S - window) / C  # only tokens outside sliding window are compressed
        base = b_kv_fp8 * (window + compressed) * d_kv
        if has_indexer:
            base += b_idx_fp4 * compressed * d_idx
        return base

    mem_c4a_mixed_1M = kv_per_layer_mixed(max_S, C_c4a, True)
    mem_c128a_mixed_1M = kv_per_layer_mixed(max_S, C_c128a, False)
    mem_swa_mixed_1M = b_kv_fp8 * min(max_S, W) * d_kv
    total_kv_1M_mixed = L_c4a * mem_c4a_mixed_1M + L_c128a * mem_c128a_mixed_1M + L_swa * mem_swa_mixed_1M
    ...
    total_mixed_128K = (
        ...
        + L_swa * b_kv_fp8 * min(131072, W) * d_kv
    )
    ...
    if total_kv_1M_mixed > 0:
        ...
        budget_kv = 40 * 1024**3
        for S in scenarios:
            swa_kv_at_S = b_kv_fp8 * min(S, W) * d_kv
            kv_total_at_S = L_c4a * kv_per_layer_mixed(S, C_c4a, True) + L_c128a * kv_per_layer_mixed(S, C_c128a, False) + L_swa * swa_kv_at_S
